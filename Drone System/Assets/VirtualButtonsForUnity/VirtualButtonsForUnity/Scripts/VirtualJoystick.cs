...
public class VirtualJoystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    [SerializeField] private string stickControlPath; // Horizontal control
    ...
    protected virtual void Awake()
    {
        ...
        {
            bgCanvasGroup.alpha = _hideOnPointerUp ? 0 : 1;
        }
    }
    ...
    public Vector3 Direction
    {
        get
        {
            // Calculate joystick direction based on the handle's position
            Vector2 joystickDirection = handle.anchoredPosition / movementRange; // Normalize based on movement range
            joystickDirection = Vector2.ClampMagnitude(joystickDirection, 1f); // Clamp to the max distance
            return new Vector3(joystickDirection.x, 0, joystickDirection.y); // x, y, z direction
        }
    }
    ...
    public void OnDrag(PointerEventData eventData)
    {
        if (joystickType == VirtualJoystickType.Floating)
        {
            Vector2 position = GetAnchoredPosition(eventData.position);
            handle.anchoredPosition = Vector2.ClampMagnitude(position - (Vector2)centerArea.anchoredPosition, movementRange);
        }
    }
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        handle.anchoredPosition = Vector2.zero; // Reset handle position on release
    }
